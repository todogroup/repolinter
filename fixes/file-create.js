...
async function fileCreate(fs, options, targets, dryRun = false) {
  ...
  if (typeof options.text === 'string')
    ...
    if (options.text.url) {
      ...
      if (!file)
        return new Result(`Could not find file matching pattern ${ options.text.file } for file-create.`, [], false)
      content = fs.getFileContents(file)
    }
  }
  if (!content)
    return new Result('Text was not specified for file-create! Did you configure the ruleset correctly?', [], false)
