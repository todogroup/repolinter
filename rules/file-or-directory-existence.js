...
function fileOrDirectoryExistence(fs, options) {
  const fileOrDirectoryExists = await fs.findFirst(options.globsAny, options.nocase)
  ...
  ? new Result(
      '',
      [{ passed: true, path: file, message: 'Found file or directory matching the specified patterns' }],
      true
    )
  : new Result(
      `${
