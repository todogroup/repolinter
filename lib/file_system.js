...
class FileSystem {
  ...
  findAll (globs) {
    return glob.sync(globs, {cwd: this.targetDir})
      .filter(relativePath => { return this.shouldInclude(relativePath) })
  }

  shouldInclude (path) {
    if (this.filterPaths.length === 0) { return true }
    return this.getFilterFiles().includes(path) || this.getFilterDirectories().some(directory => path.startsWith(directory))
  }
