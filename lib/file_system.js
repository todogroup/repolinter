...
class FileSystem {
   ...
   * @returns {undefined | string} A path relative to this.targetDir, or undefined if no items were found
   */
   ...
   * @param {string | Array<string>} globs The globs to search with
   * @param {boolean} nocase Whether or not to ignore case-sensitivity
   * @returns {Array<string>} A list of paths relative to this.targetDir
   */
  ...
  isBinaryFile (relativeFile) {
    ...
    try {
      return isBinaryFile.isBinaryFileSync(file)
    } catch (e) {
   ...
   * @returns {string | undefined} A string with the file contents, or undefined if the file is not found. 
   */
  ...
  setFileContents(relativeFile, contents) {
    fs.writeFileSync(path.resolve(this.targetDir, relativeFile), contents);
  }
